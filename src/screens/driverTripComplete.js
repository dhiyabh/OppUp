...
export default class DriverTripComplete extends React.Component {
    ...
    sendPushNotification2(customerUID, bookingId) {
        ...
        customerRoot.once('value', customerData => {
            if (customerData.val()) {
                let allData = customerData.val()
                RequestPushMsg(allData.pushToken ? allData.pushToken : null, languageJSON.driver_completed_ride + bookingId)
            }
        })
    }
        ...
        return (
            <View style={styles.mainViewStyle}>
                ...
                <View style={styles.dateViewStyle}>
                    <Text style={styles.dateViewTextStyle}>{new Date(this.state.rideDetails.tripdate).toLocaleString(dateStyle)}</Text>
                </View>
                ...
                <View style={styles.addressViewStyle}>
                    <FlatList
                        data={[
                            { key:'key1', value:this.state.rideDetails.pickup.add, type: 'pickup' },
                            { key:'key2', value:this.state.rideDetails.drop.add, type: 'drop' }
                        ]}
                        renderItem={({ item }) =>
                            <View style={styles.pickUpStyle}>
                                ...
                                <Text style={styles.addressViewTextStyle}>{item.value}</Text>
                            </View>
                        }
                    />
                </View>
                <View style={styles.confBtnStyle}>
                    <Button
                        title={this.state.rideDetails.booking_type_web?languageJSON.complete_ride:languageJSON.request_payment}
